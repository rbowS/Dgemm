    ...
    for (int i = tile_K-1; i >= 0; i--)
    {
        ...
        for (int j = 0; j < 16; j++)
        {
            ...
            for (int k = 0; k < 4; k++)
            {
                for (int l = 0; l < 4; l++)
                {
                    fragA[k][l] = sh_tile_A[fragAid*4+k][j*4+l];
                }
            }
            //load frag B to register
            for (int k = 0; k < 4; k++)
            {
                for (int l = 0; l < 4; l++)
                {
                    fragB[k][l] = sh_tile_B[j*4+k][fragBid*4+l];
                }
            }
                              ...
                              const int M, const int K, const int N)
{
    ...
    const int warp_id = blockIdx.x/32;
    const int line_id = tid%32;
    ...
    for (int i = tile_K-1; i >= 0; i--)
    {
        ...
        int inn_aid = warp_id%4;
        int inn_bid;
        if(warp_id < 4)
            inn_bid = warp_id;
        if(warp_id >= 4)
            inn_bid = (warp_id-4 + 2) % 4;
        int inn_row_aid = (inn_aid/2)*32;
        int inn_col_aid = (inn_aid%2)*32;
        int inn_row_bid = (inn_bid/2)*32;
        int inn_col_bid = (inn_bid%2)*32;
        for(int j = 0; j < 32; j++)
        {
            for(int k=0; k<32; k++)
            {
                int tmpc = sh_tile_A[inn_row_aid+j][inn_col_aid+k]*sh_tile_B[inn_row_bid+j][inn_col_bid+k];
                __syncwarp();
                for (int offset = blockDim.x / 2; offset > 0; offset >>= 1)  
                    tmpc += __shfl_down_sync(0xffffffff, tmpc, i);
                if(line_id == 0)
                    atomicAdd(&sh_tile_C[inn_row_aid+j][inn_col_bid+k], tmpc);
                __syncwarp();
            }
        }
